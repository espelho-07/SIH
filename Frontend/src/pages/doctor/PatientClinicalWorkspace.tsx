...
export const PatientClinicalWorkspace: React.FC = () => {
  ...
  // Mobile Workspace View Toggle (Active Rx vs History)
  const [mobileWorkspaceView, setMobileWorkspaceView] = useState<'ENCOUNTER' | 'HISTORY'>('ENCOUNTER');
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ================================================== */}
      {/* MOBILE WORKSPACE SWITCHER (Doctor Mobile Experience) */}
      {/* ================================================== */}
      <div className="lg:hidden flex rounded-xl bg-slate-100 p-1 mb-2 border border-slate-200">
        <button
          type="button"
          onClick={() => setMobileWorkspaceView('ENCOUNTER')}
          className={cn(
            'flex-1 py-2 text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-1.5',
            mobileWorkspaceView === 'ENCOUNTER'
              ? 'bg-white text-teal-800 shadow-xs'
              : 'text-slate-500 hover:text-slate-800'
          )}
        >
          <Stethoscope className="h-3.5 w-3.5" />
          <span>Active Treatment (Rx)</span>
        </button>
        <button
          type="button"
          onClick={() => setMobileWorkspaceView('HISTORY')}
          className={cn(
            'flex-1 py-2 text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-1.5',
            mobileWorkspaceView === 'HISTORY'
              ? 'bg-white text-teal-800 shadow-xs'
              : 'text-slate-500 hover:text-slate-800'
          )}
        >
          <FileText className="h-3.5 w-3.5" />
          <span>Patient History & Labs</span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-start">
        ...
        {/* ========================================== */}
        <div className={cn('lg:col-span-4 space-y-3', mobileWorkspaceView !== 'HISTORY' && 'hidden lg:block')}>
        ...
        {/* ========================================== */}
        <div className={cn('lg:col-span-8 space-y-4', mobileWorkspaceView !== 'ENCOUNTER' && 'hidden lg:block')}>
