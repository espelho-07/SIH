...
1. RELEVANCE & ACCURACY:
   ...
   - If the user asks about MEDICINE availability or stock, talk ONLY about the medicine, its stock in our nearest hospital / medical store, and safe medicine use. DO NOT start diagnosing diseases or bringing up unrelated illnesses like diverticulitis or pneumonia!
   - If the user asks about HOSPITALS, MEDICAL STORES, or NEARBY FACILITIES, show ONLY the single closest / nearest hospital and/or nearest medical store to the user's location. Do not list all facilities in the district. State its exact distance in km, address, bed capacity, phone number, and operating hours.
...
7. MANDATORY AI MEDICAL DISCLAIMER / WARNING (MUST BE INCLUDED IN EVERY SINGLE CHAT):
   At the very end of EVERY response without exception, include a clear medical disclaimer:
   - For Gujarati responses:
     > ⚠️ **તબીબી ચેતવણી (Medical Disclaimer)**: હું એક AI ચેટબોટ છું અને વાસ્તવિક ડૉક્ટર નથી. AI દ્વારા મળતી માહિતી ૧૦૦% સચોટ કે સંપૂર્ણ ન પણ હોઈ શકે. સચોટ નિદાન, દવાઓ અને યોગ્ય સારવાર માટે કૃપા કરીને હંમેશા લાયકાત ધરાવતા ડૉક્ટર અથવા નજીકના સરકારી/ખાનગી આરોગ્ય કેન્દ્રનો સંપર્ક કરવો.
...
class HealthcareRAGEngine:
    ...
    def retrieve_context(
        ...
        # 5. Hospital Facility lookup (NEAREST ONLY)
        if intent in ["HOSPITAL_QUERY", "EMERGENCY", "SYMPTOM_DIAGNOSIS"] or is_emergency or "hospital" in query_text.lower() or "phc" in query_text.lower() or "near" in query_text.lower():
            emergency_only = is_emergency or intent == "EMERGENCY"
            hospitals = []
            if latitude is not None and longitude is not None:
                hospitals = mongo_manager.find_nearby_hospitals(lat=latitude, lng=longitude, limit=1)
            if not hospitals:
                search_q = query_text if intent not in ["SYMPTOM_DIAGNOSIS", "EMERGENCY"] else None
                hospitals = mongo_manager.search_hospitals(query=search_q, district=district, emergency_only=emergency_only, limit=1)
            if not hospitals:
                hospitals = mongo_manager.search_hospitals(district=district, limit=1)
    ...
    def generate_response(
        ...
        # Enforce mandatory medical AI disclaimer on all responses
    ...
    def _ensure_medical_disclaimer(self, answer: str, user_query: str) -> str:
        """Ensure all assistant answers contain a prominent medical AI disclaimer."""
        if not answer:
            return answer

        # Check if user query or answer contains Gujarati script
        has_gujarati = any(0x0A80 <= ord(char) <= 0x0AFF for char in (user_query + answer))

        disclaimer_markers = [
            "medical disclaimer", "disclaimer", "તબીબી ચેતવણી", "અસ્વીકરણ",
            "ai chatbot", "ai ચેટબોટ", "not a doctor", "ડૉક્ટર નથી", "consult a doctor", "લાયકાત ધરાવતા ડૉક્ટર"
        ]
        
        lower_ans = answer.lower()
        if any(marker in lower_ans for marker in disclaimer_markers):
            return answer

        if has_gujarati:
            disclaimer = (
                "\n\n> ⚠️ **તબીબી ચેતવણી (Medical Disclaimer)**: હું એક AI ચેટબોટ છું અને વાસ્તવિક ડૉક્ટર નથી. "
                "AI દ્વારા પૂરી પાડવામાં આવેલી વિગતો ૧૦૦% સચોટ કે સંપૂર્ણ ન પણ હોઈ શકે. "
